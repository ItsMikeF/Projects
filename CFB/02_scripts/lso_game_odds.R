...
# Updated function to retrieve NCAAF odds with spreads and totals
get_ncaaf_odds <- function(api_key, target_date = today(), 
  ...
  odds_df <- map_dfr(content, function(game) {
    ...
    # If no match found (edge case), default to first/second outcome to avoid errors
...
if (api_key == "") {
  stop("API key not found. Please set 'LSO_KEY' in your .Renviron file.")
}
