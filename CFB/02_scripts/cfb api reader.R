...
cfbd_to_pff_abbrev_df <- read.csv("./01_data/cfbd_to_pff_abbrev.csv")
...
for (week in 6) {
  ...
  for (team_id in cfbd_to_pff_abbrev_df$pff_team_id) {
    ...
    # Make the request with cookies passed using do.call
    response <- do.call(GET, c(list(url = url, query = list(league = "ncaa")), set_cookies = cookies_pff))
    ...
    if (status_code(response) == 200) {
      ...
      cat("Request failed for team_id", team_id, "week", week, "with status code", status_code(response), "\n")
    }
  }
}
