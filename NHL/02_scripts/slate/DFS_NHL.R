...
# 1.0 load packages, options, and date----------------------------------------
...
# round calculations to 3 digits
options(digits = 3)
...
slate_data <- function(){
  ...
  team_abbrev_recode <- c("CLS" = "CBJ", "NJ" = "LA", "LAK" = "SJS", "MON" = "MTL")
  ...
  # get games
  games <<- nhl_salaries %>% select(Away, Home, Time) %>% distinct()
...
powerplay <- function(){
  ...
  pp <<- cbind(pp_slate_away, pp_slate_home) %>% 
    mutate(away_home = away_xGoalsFor_per60 - home_xGoalsAgainst_per60, 
           home_away = home_xGoalsFor_per60 - away_xGoalsAgainst_per60) %>% 
    relocate(c(away_home, home_away), .after = away)
}
powerplay()

# teams
slate_teams <- function(){
  teams <- mp_teams %>% 
    filter(situation == "all") %>% 
    select(name, 
           games_played,
  ...
  # lines
  slate_lines <- function(){
    
    team_lines <- lapply(nhl_team_logos$team_abbr[1:32], function(x){
      mp_lines %>% 
        filter(team == x & position == "line") %>% 
  ...
  teams <- teams %>% 
    ...
    left_join(nhl_team_logos, by = c("name" = "team_abbr"))
}
slate_teams()
