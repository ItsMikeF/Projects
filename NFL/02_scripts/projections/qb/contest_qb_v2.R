    ...
    pff_pass <- function(){
      ...
      receiving_summary <<- read.csv(glue("{folder}/pff/receiving_summary.csv")) %>%
        mutate(player = clean_player_names(player, lowercase = T), 
               ...
               td_game = round(touchdowns / player_game_count, digits = 1),
               rec_yards_game = round(yards / player_game_count, digits = 1) )
...
# train random forest model
qb_passing_yards_rf <- randomForest(passing_yards ~  
...
# use qb_passing_yards_rf to predict on test data
qb_passing_yards_rf_predictions <- round(predict(qb_passing_yards_rf, test_data), digits = 2)

# evaulated predictions 
qb_passing_yards_rf_performance <- round(postResample(qb_passing_yards_rf_predictions, test_data$passing_yards), digits = 3)
qb_passing_yards_rf_performance
...
# Train the model with caret
qb_passing_yards_rf_tuned <- train(
  passing_yards ~  
    spread + total_line + dome + #temp + wind + # game data
    pyards_game + ypa + td_game + # pass game
    #ryards_game +  # # rush game
...
# View the best model
print(qb_passing_yards_rf_tuned$bestTune)

# Dotchart of variable importance as measured by a Random Forest
varImpPlot(qb_passing_yards_rf)
...
# use qb_pass_touchdown_rf to predict on test data
qb_pass_touchdown_rf_predictions <- round(predict(qb_pass_touchdown_rf, test_data), digits = 2)

# evaulated predictions 
qb_pass_touchdown_rf_performance <- round(postResample(qb_pass_touchdown_rf_predictions, test_data$pass_touchdown), digits = 3)
qb_pass_touchdown_rf_performance
...
# Train the model with caret
qb_pass_touchdown_rf_tuned <- train(
  pass_touchdown ~  
    spread + total_line + dome + #temp + wind + # game data
    pyards_game + ypa + td_game + # pass game
    #ryards_game +  # # rush game
...
# View the best model
print(qb_pass_touchdown_rf_tuned$bestTune)

# Dotchart of variable importance as measured by a Random Forest
varImpPlot(qb_pass_touchdown_rf)
...
# use qb_pass_attempt_rf to predict on test data
qb_pass_attempt_rf_predictions <- round(predict(qb_pass_attempt_rf, test_data), digits = 2)

# evaulated predictions 
qb_pass_attempt_rf_performance <- round(postResample(qb_pass_attempt_rf_predictions, test_data$pass_attempt), digits = 3)
qb_pass_attempt_rf_performance
...
# Train the model with caret
qb_pass_attempt_rf_tuned <- train(
  pass_attempt ~  
    spread + total_line + dome + #temp + wind + # game data
    pyards_game + ypa + td_game + # pass game
    #ryards_game +  # # rush game
...
# View the best model
print(qb_pass_attempt_rf_tuned$bestTune)

# Dotchart of variable importance as measured by a Random Forest
varImpPlot(qb_pass_attempt_rf)
