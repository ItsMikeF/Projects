...
which(grepl("2022_w8", contest_files))
indices

contest_files[indices]
contest_files <- contest_files[indices]
contest_files <- contest_files[1:7]
contest_files
...
contests_rb <- lapply(contest_files, function(x){
  ...
  x = glue("./01_data/contests/{x}")
  
  salaries <- read.csv(glue("{x}/DKSalaries.csv")) %>% 
    select(1,3,6:8) %>% 
  ...
  rushing_summary <- read.csv(glue("{x}/pff/rushing_summary.csv"))
  
  chart_oline_dline_matchup <- read.csv(glue("{x}/pff/oline_dline_matchup_chart.csv"))
