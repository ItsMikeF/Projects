...
nfl_pff_dk_ownership <- read.csv("dk-ownership.csv")
nfl_pff_projections <- read.csv("projections.csv")

nfl_pff_passing_summary <- read.csv("passing_summary.csv")
nfl_pff_passing_concept <- read.csv("passing_concept.csv")
nfl_pff_passing_pressure <- read.csv("passing_pressure_blitz.csv")

nfl_pff_receiving_summary <- read.csv("receiving_summary.csv")
nfl_pff_receiving_scheme <- read.csv("receiving_scheme.csv")

nfl_pff_rb <- read.csv("rushing_summary.csv")

nfl_pff_line_pass_blocking_efficiency <- read.csv("line_pass_blocking_efficiency.csv")
...
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'ARZ', 'ARI')
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'BLT', 'BAL')
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'CLV', 'CLE')
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'HST', 'HOU')
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'JAX', 'JAC')
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency == 'LA', 'LAR')
nfl_pff_line_pass_blocking_efficiency$pbe_rank <- round(rank(-nfl_pff_line_pass_blocking_efficiency$pbe), digits = 0)
nfl_pff_line_pass_blocking_efficiency$pbe_sd <- round((nfl_pff_line_pass_blocking_efficiency$pbe - mean(nfl_pff_line_pass_blocking_efficiency$pbe, na.rm=T)) / sd(nfl_pff_line_pass_blocking_efficiency$pbe, na.rm = T), digits = 2)
nfl_pff_line_pass_blocking_efficiency <- replace(nfl_pff_line_pass_blocking_efficiency, nfl_pff_line_pass_blocking_efficiency =='JAC','JAX')
...
nfl_pff_receiving_summary <- replace(nfl_pff_receiving_summary, nfl_pff_receiving_summary =='D.K. Metcalf','DK Metcalf')
...
nfl_qb <- nfl_pff_passing_summary %>% 
  left_join(nfl_salaries, by = c('player' ='Name'))
         
nfl_qb <- nfl_qb %>% 
  left_join(nfl_pff_dk_ownership, by = c('Name' = 'player')) 

nfl_qb <- nfl_qb %>% 
  left_join(nfl_pff_line_pass_blocking_efficiency, by = c('TeamAbbrev' = 'team_name'))

nfl_qb <- nfl_qb %>% 
  left_join(nfl_pff_passing_concept, by = c('Name' = 'player'))

player <- nfl_pff_passing_pressure$player
blitz_dropbacks_percent <- nfl_pff_passing_pressure$blitz_dropbacks_percent / 100
blitz_grades_pass <- nfl_pff_passing_pressure$blitz_grades_pass
...
nfl_qb$dropbacks_game_sd <- round((nfl_qb$dropbacks_game - mean(nfl_qb$dropbacks_game, na.rm=T)) / sd(nfl_qb$dropbacks_game, na.rm = T), digits = 2)

nfl_qb$pressure_to_sack_rate_rank <- rank(nfl_qb$pressure_to_sack_rate)
nfl_qb$pressure_to_sack_rate_sd <- round((nfl_qb$pressure_to_sack_rate - mean(nfl_qb$pressure_to_sack_rate, na.rm=T)) / sd(nfl_qb$pressure_to_sack_rate, na.rm = T), digits = 2)
...
nfl_qb$sum_sd <- round(
  ...
  (0.10 * nfl_qb$dropbacks_game_sd)
  (0.20 * nfl_qb$def_pass_epa_sd) + 
  (0.10 * (nfl_qb$total_rec_salary_sd - nfl_qb$cov_sd)) +
  (0.20 * nfl_qb$blitz_grades_pass_sq_exp_blitz_rate_sd) +
  (0.10 * (nfl_qb$pbe_sd - nfl_qb$prsh_sd - nfl_qb$pressure_to_sack_rate_sd)) , digits =3)
...
nfl_qb %>%
  select(Name,
         ...
         pressure_to_sack_rate_rank,
         pressure_to_sack_rate_sd,
         total_rec_salary,
         total_rec_salary_sd,
         cov_sd,
         name_salary_own) %>%
...
nfl_wr <- nfl_salaries %>%
  left_join(nfl_pff_receiving_summary, by = c('Name' = 'player'))
...
nfl_te <- nfl_te %>% 
  left_join(nfl_pff_dk_ownership, by = c('offPlayer' = 'player'))
