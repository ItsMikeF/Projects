...
dg <- read.csv("dg.csv")
data <- dg %>% 
  select(player_name, salary, open_odds, close_odds, odds_delta, odds_delta_per, make_cut, top_20, top_10, top_5, top_3, win, 
         total_pts) %>% 
  drop_na(close_odds) %>% 
  filter(odds_delta_per != 0)

data <- dg %>% 
  select(salary, close_odds, win, finish_pts) %>% 
  drop_na(close_odds) %>% 
  drop_na(win)
...
parts = createDataPartition(data$finish_pts, p = .8, list = F)
train = data[parts, ]
test = data[-parts, ]
...
error_chart[1,1] <- round(mean((test_y - pred_y)^2), digits = 1) #mse
error_chart[2,1] <- round(caret::MAE(test_y, pred_y), digits = 1) #mae
error_chart[3,1] <- round(caret::RMSE(test_y, pred_y), digits = 1) #rmse
